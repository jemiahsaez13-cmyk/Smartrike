...
export const PasswordResetCodeScreen = () => {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      setError(verificationError?.message || 'Password-reset verification failed.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      await notify('Code sent', 'A new password-reset code has been sent. The previous code is no longer valid.');
    } catch (resendError: any) {
      setError(resendError?.message || 'A new password-reset code could not be sent.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <Text style={styles.subtitle}>
            If an account can receive password-reset email, a 6-digit code was sent to{`\n`}
          ...
          <TouchableOpacity style={styles.changeButton} onPress={() => navigation.navigate('ForgotPassword')}>
            <Text style={styles.changeText}>Use a different email address</Text>
          </TouchableOpacity>
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  changeButton: { minHeight: 48, alignItems: 'center', justifyContent: 'center' },
  changeText: { ...typography.bodySmall, color: colors.textSecondary },
});
